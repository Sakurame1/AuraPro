...
const getExeDir = (): string => {
  ...
  return path.dirname(app.getAppPath()) // points to desktop-main
}

export const getUserHomePath = (): string => {
  return path.normalize(path.join(getExeDir(), 'cache', 'home'))
}

export const getUserDataPath = (): string => {
  const userDataDir = getExeDir()
  return path.normalize(userDataDir)
}

export const getInstallDir = (): string => {
  const installDir = getExeDir()
  ...
  try {
    ptyProcess = pty.spawn(pythonPath, commandArgs, {
      ...
      env: {
        ...
        ...(configEnvVars ?? {}),
        DATA_DIR: dataDir,
        WEBUI_SECRET_KEY: 'Aura_Pro_secret_key_2026',
        WEBUI_NAME: 'AURA Pro',
        HF_HOME: path.join(getInstallDir(), 'cache', 'huggingface'),
        TIKTOKEN_CACHE_DIR: path.join(getInstallDir(), 'cache', 'tiktoken'),
        HOME: path.join(getInstallDir(), 'cache', 'home'),
        USERPROFILE: path.join(getInstallDir(), 'cache', 'home'),
        PYTHONUNBUFFERED: '1',
        ...(process.platform === 'win32' ? { PYTHONIOENCODING: 'utf-8' } : {})
      }
    })
...
const DEFAULT_CONFIG: AppConfig = {
  version: 1,
  defaultConnectionId: 'local-aurapro',
  connections: [
    {
      id: 'local-aurapro',
      name: 'AURA Pro Local Server',
      type: 'local',
