<script lang="ts">
  ...
  const startModelDownload = async () => {
    ...
    try {
      if (selectedModel.name === 'low_EQ4.gguf') {
        const current = await window.electronAPI.getConfig()
        const llamaCpp = { 
          ...(current.llamaCpp || {}), 
          ctxSize: 8192,
          ...
          {#each AURA_MODELS as model}
            ...
            >
              <div class="flex flex-col">
                <span class="text-[13px] font-medium {selectedModel.name === model.name ? 'text-emerald-400' : 'opacity-80'}">{model.name.replace('.gguf', '')}</span>
                <span class="text-[10px] opacity-30">{model.sizeStr} · {model.ramInfo}</span>
              </div>
